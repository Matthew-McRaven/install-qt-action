...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            switch (process.platform) {
                case "win32": {
                    yield exec.exec(`${pythonName} -m aqt tool windows`, iargs);
                    yield exec.exec(`${pythonName} -m aqt tool windows`, msvcargs);
                }
                case "darwin": {
                    yield exec.exec(`${pythonName} -m aqt tool mac`, iargs);
                }
                default: {
                    yield exec.exec(`${pythonName} -m aqt tool linux`, iargs);
                }
            }
